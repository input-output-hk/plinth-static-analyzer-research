...
This document summarizes findings from **9 Plinth audits** in the Cardano ecosystem.
...
- **MSW-B01. Dangerous Use of unstableMakeIsData**: Using unstableMakeIsData assigns constructor indices based on declaration order, which can change unpredictably and break deployed contracts.<br><ins>Detectable pattern</ins>: usage of unstableMakeIsData function

**May be relevant**

- **MSW-A01. Double Satisfaction Attack**: Validator aggregates payment outputs by checking only recipient's address without verifying uniqueness. Allows attackers to satisfy multiple orders with single payment by batching orders from the same creator.<br><ins>Detectable pattern</ins>: value aggregation filtering by address without datum uniqueness check [DOUBLE-SATISFACTION]
...
- **AADA-004: Resource DoS attack**: Validator doesn't restrict which tokens can be included in output UTxOs, allowing attackers to bloat outputs with arbitrary tokens.<br><ins>Detectable pattern</ins>: subset value validation instead of equality check [TRASH-TOKENS]
- **AADA-104: Burning of oracle tokens can liquidate**: Liquidation logic allows oracle tokens to be burned instead of minted, and the default oracle policy does not properly enforce ownership or signatures, enabling arbitrary liquidations.<br><ins>Detectable pattern</ins>: incomplete validation of token tuple components (cs, tn, n) [INCOMPLETE-TOKEN-VALIDATION]
- **AADA-303: Liquidation token minted check too lenient**: Liquidation validation only checks for minting of any token under a currency symbol, not a specific token name, allowing forged liquidation tokens.<br><ins>Detectable pattern</ins>: incomplete validation of token tuple components (cs, tn, n) [INCOMPLETE-TOKEN-VALIDATION]
- **AADA-305: Oracle policy not checking destination of all tokens**: The oracle policy only checks that some oracle token reaches the destination address, allowing other oracle tokens to be minted or moved elsewhere without restriction.<br><ins>Detectable pattern</ins>: No token destination validation for all tokens. [MISSING-ADDRESS-VALIDATION]
- **AADA-403: possible minting of liquidation tokens with different token name**: The oracle policy permits minting additional liquidation tokens with arbitrary token names alongside the legitimate one, weakening assumptions about uniqueness.<br><ins>Detectable pattern</ins>: incomplete validation of token tuple components (cs, tn, n) [INCOMPLETE-TOKEN-VALIDATION]

**May be relevant**

- **AADA-002: Double Satisfaction - Collateral**: Validator aggregates payment outputs by checking only recipient's address without verifying uniqueness. A lender can batch multiple loan requests from the same borrower in a single transaction, providing only the maximum collateral amount instead of the sum.<br><ins>Detectable pattern</ins>: value aggregation filtering by address without address uniqueness check [DOUBLE-SATISFACTION]
- **AADA-003: Double Satisfaction - Loan**: Validator aggregates payment outputs by checking only recipient's address without verifying uniqueness. A lender can batch multiple loan requests from the same borrower in a single transaction, providing only the maximum loan amount instead of the sum.<br><ins>Detectable pattern</ins>: value aggregation filtering by address without address uniqueness check [DOUBLE-SATISFACTION]
- **AADA-202. Losing partial interest precision**: Unnecessary intermediary rounding in interest calculation causes lenders to lose money.<br><ins>Detectable pattern</ins>: division before multiplication (elapsed / total) _ amount loses precision compared to (elapsed _ amount) / total. [PRECISION-LOSS]
- **AADA-302. Rounding of interest favors borrower**: Interest calculations rely on repeated integer division with flooring, systematically underpaying lenders over time.<br><ins>Detectable pattern</ins>: Computation using integer division without explicit rounding (floor or ceil). Static analyzer could detect rounding direction but determining correct direction requires business context
- **AADA-304. Constant protocol staking key rewards capped**: Staking credential hardcoded in contract parameters rather than variable in datum causes reward saturation once pool reaches cap.<br><ins>Detectable pattern</ins>: staking key in immutable parameters and not as a variable datum. [IMMUTABLE-CREDENTIAL]
- **AADA-402. Inverted percentage calculation**: Two inverted divisions in separate functions accidentally cancel out (a/b) used as divisor → multiply by (b/a).<br><ins>Detectable pattern</ins>: reciprocal cancellation, but requires cross-function data flow analysis.
...
- **AADA-101: Borrower may replay loan without paying interest**: There is insufficient validation that checks that interestPayPDate redeemer is a sufficiently accurate time approximation for the use case. Requires understanding the purpose of redeemers
- **AADA-102: Lender can collect interest immediately**: There is insufficient validation that checks that checks that lendDate redeemer is a sufficiently accurate time approximation for the use case. Requires understanding the purpose of redeemers
...
- **2.2.0.8. Implementation of equality test for PoolDatum is incomplete**: Eq instance for PoolDatum omits pdPrice field from equality check, comparing only subset of fields. Requires understanding if omission is intentional design and semantic understanding of domain model
...
- **ID-01. Incomplete Currency Symbol Validation**: Function validates token name matches but doesn't properly check currency symbol, allowing tokens with same name but different policies to pollute UTxOs.<br><ins>Detectable pattern</ins>: incomplete validation of token tuple components (cs, tn, n) [INCOMPLETE-TOKEN-VALIDATION]
- **ID-02. Unvalidated Reference Script Field**: Protocol outputs don't validate or check reference script field, allowing arbitrary reference scripts to be attached and increase future transaction fees.<br><ins>Detectable pattern</ins>: output validation logic that ignores reference script field entirely [UNVALIDATED-REFERENCE-SCRIPT]
